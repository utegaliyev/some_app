...
export interface IPersonalInfoClient {
    create(command: RegisterPersonalInfoCommand): Observable<string>;
}

@Injectable({
    providedIn: 'root'
})
export class PersonalInfoClient implements IPersonalInfoClient {
    ...
    create(command: RegisterPersonalInfoCommand): Observable<string> {
        let url_ = this.baseUrl + "/api/PersonalInfo";
        url_ = url_.replace(/[?&]$/, "");
...
export interface IDictionaryClient {
    ...
    getProvinces(countryId: number | undefined): Observable<ProvinceDto[]>;
}

@Injectable({
    providedIn: 'root'
})
export class DictionaryClient implements IDictionaryClient {
    private http: HttpClient;
    private baseUrl: string;
    protected jsonParseReviver: ((key: string, value: any) => any) | undefined = undefined;
    ...
    getProvinces(countryId: number | undefined): Observable<ProvinceDto[]> {
        let url_ = this.baseUrl + "/api/Dictionary/Provinces?";
        if (countryId === null)
...
export class RegisterPersonalInfoCommand implements IRegisterPersonalInfoCommand {
    login!: string;
    password!: string;
    ...
    constructor(data?: IRegisterPersonalInfoCommand) {
        if (data) {
    ...
    init(_data?: any) {
        if (_data) {
            ...
            this.isAgreed = _data["isAgreed"];
            this.provinceId = _data["provinceId"];
        }
    }

    static fromJS(data: any): RegisterPersonalInfoCommand {
        data = typeof data === 'object' ? data : {};
        let result = new RegisterPersonalInfoCommand();
        result.init(data);
        return result;
    }
...
export interface IRegisterPersonalInfoCommand {
    login: string;
    password: string;
    confirmPassword: string;
    isAgreed?: boolean;
